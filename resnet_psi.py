...
def psi_step(field, velocity, sources, active):
    """
    Um step da equação de onda modificada:
...
def compute_crystal_maps(X, PG, field_size=FIELD_SIZE, bs=64):
    """
...
if __name__ == '__main__':
    ...
    rn = ResNetPsi()
    rn.fit(train_imgs, train_labels)
    acc = rn.score(test_imgs, test_labels)
    print(f"\n  Acurácia: {acc:.1f}%")
    ...
    print("Extraindo crystal_maps...")
    t0 = time.time()
    PG = build_gaussians()
    cmaps_all  = compute_crystal_maps(train_imgs, PG)
    cmaps_test = compute_crystal_maps(test_imgs, PG)
    print(f"  Pronto: {time.time()-t0:.0f}s")
