"""
Auditoria 9: Fashion-MNIST — Protótipos Cristalinos

Mesmo mecanismo do MNIST, agora com roupas.
Prova que o ResNet-Ψ é genérico, não específico pra dígitos.
...
N_PROTO_LIST = [10, 50, 100, 500, 1000, 5000]
N_TEST = 500

print(f"\n{'='*70}")
print("AUDITORIA 9: Fashion-MNIST — Protótipos Cristalinos")
print(f"Teste: {N_TEST} imagens | Zero treino")
print(f"{'='*70}")
...
for n_proto in N_PROTO_LIST:
    ...
    prototypes = {}
    for cls in range(10):
        prototypes[cls] = class_cmaps[cls].mean(dim=0)
    ...
    for cls in range(10):
        ...
        print(f"    {CLASS_NAMES[cls]:>10}: {n_crys:.0f} cristais")

    # Distância euclidiana (melhor método no MNIST)
    correct_dist = 0
...
print(f"\n{'='*70}")
print("RESULTADO FINAL: Fashion-MNIST")
print(f"{'='*70}")
print(f"\n{'N exemplos':>12} {'Dist euclid':>13} {'Corr direta':>13}")
print("-"*42)
for r in all_results:
    print(f"  {r['n_proto']:>10}   {r['acc_dist']:>9.1f}%   {r['acc_direct']:>9.1f}%")

print(f"\nReferência: aleatório = 10.0%")
print(f"Referência: MNIST protótipos (5000 ex) = 77.4%")
...
fig.suptitle('Auditoria 9 — Fashion-MNIST: Protótipos Cristalinos (zero treino)', fontsize=13)
...
ax.axhline(y=77.4, color='orange', linestyle='--', alpha=0.5, label='MNIST protótipos (77.4%)')
ax.set_xlabel('Exemplos por protótipo')
ax.set_ylabel('Acurácia (%)')
ax.set_title('Fashion-MNIST: Acurácia vs N exemplos')
ax.legend(fontsize=7)
ax.grid(alpha=0.3)

# Protótipos diferenciais
ax = axes[1]
ax.set_title('Protótipos diferenciais Fashion-MNIST')
ax.axis('off')
for i in range(10):
    row, col = i // 5, i % 5
    ...
    sub.set_title(CLASS_NAMES[i], fontsize=6)
    sub.axis('off')

plt.savefig('viz_audit_9_fashion.png', dpi=130, bbox_inches='tight')
plt.close()
print(f"\n-> viz_audit_9_fashion.png")
print("Pronto.")
